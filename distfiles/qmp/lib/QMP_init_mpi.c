...
static void BGQtoLogical4d_ABECD(int * xyzt, int *Lxyzt, int *abcde, int *Labcde)
{
  ...
  if ( abcde[E] == 0 ) xyzt[Y] = abcde[B];
  else                 xyzt[Y] = 2*Labcde[B]-1-abcde[B];

}

static void BGQtoLogical4d_AEBCD(int * xyzt, int *Lxyzt, int *abcde, int *Labcde)
{
  Lxyzt[X] = Labcde[A]*Labcde[E];
  Lxyzt[Y] = Labcde[B];
  Lxyzt[Z] = Labcde[C];
  Lxyzt[T] = Labcde[D];
  ...
  if ( abcde[E] == 0 ) xyzt[X] = abcde[A];
  else                 xyzt[X] = 2*Labcde[A]-1-abcde[A];
...
QMP_init_machine_i(int* argc, char*** argv)
{
  ...
  if(nd<=0) {
	...
	permute(QMP_global_m->coord, QMP_global_m->amap, nd);
      ...
      nd = QMP_global_m->ndim;
      QMP_global_m->amap=0;
    } else if ( c && strcmp(c, "nativeAEBCD")==0 ) {
      bgq3d=2;
      set_native_machine();
      nd = QMP_global_m->ndim;
      QMP_global_m->amap=0;
