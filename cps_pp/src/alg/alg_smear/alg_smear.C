...
void three_staple( GaugeField& gf,  Matrix& link , 
                   int *pos, int u, int orth )
{
  Matrix acumulate_mp; acumulate_mp.ZeroMatrix();
  int dir[3];
  ...
  for ( int v=0; v<8; v++ )
    {      
      ...
      gfPathOrdProdPlus(acumulate_mp, gf, pos, dir, 3); 
    }
  // 18 is the matrix size
  moveMem((Float *) &link, (Float*)&acumulate_mp, 18*sizeof(Float));
}
...
#pragma omp parallel for
  for(int i = 0; i < GJP.VolNodeSites(); ++i)
  {
    ...
    for(int mu = 0; mu < 4; ++mu)
    {
      ...
      Matrix & link(* (lat.GaugeField() + lat.GsiteOffset(x) + mu));
      
      Matrix stap;
      three_staple(gf, stap, x, mu, orthog);
      link *= 1.0 - c;
      stap *= c / 6.;
      link += stap;
