...
//*******************************************************
void metropolis_kernel(  Float *sigma, Float *U, double beta, int rgen_pos_4d)
{
//  LRG.SetInterval(1,-1);

int	NHITS = 10;
Float	SMALL = 0.2;
Float	EPSILON[MATRIXSIZE];
Float	MTEMP1[MATRIXSIZE];
Float	MTEMP2[MATRIXSIZE];

VRB.Debug(" Beta = %f\n", beta);
  Float fGamma = (2./3.) * beta;
 ...
 NHITS, SMALL );
  ...
  for(int HIT_CNT=0; HIT_CNT<NHITS; HIT_CNT++)
  {

    {
      ...
      a = SMALL * LRG.Urand_threadsafe(rgen_pos_4d); if(a>0)a*=-1; a+=1.0;
      b = LRG.Urand_threadsafe(rgen_pos_4d);
      c = LRG.Urand_threadsafe(rgen_pos_4d);
      d = LRG.Urand_threadsafe(rgen_pos_4d);
...
VRB.Debug("normalized:   a=%+6.5f b=%+6.5f c=%+6.5f d=%+6.5f\n",a,b,c,d);
      ...
      select = LRG.Urand_threadsafe(rgen_pos_4d);
...
VRB.Debug("Accept chance %e\n", accept_probability );
    ...
    die_roll = LRG.Urand_threadsafe(rgen_pos_4d);
...
//*******************************************************
void  cmhb_kernel( Float *sigma, Float *u, double beta)
{
//    LRG.SetInterval(1, -1);
